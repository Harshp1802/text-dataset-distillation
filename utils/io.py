...
def txt_results(state, steps, expr_dir):
    ...
    if expr_dir is None:
        ...
        if('avg' in state.glove.embedding_file):
            txt_name_fmt += '_bert'
        txt_name_fmt += '.txt'
    ...
    for i, (data, labels, lr) in enumerate(steps):
        ...
        sentences = []
        sentences_idx = []
        for n, (dist_sentence) in tenumerate(data):
            sentence_idx = []
            sentence=[]
            for dist_word in dist_sentence:
                word, word_idx = datasets.closest_words(dist_word, state.glove, n=0) #dist_word.cpu()
                sentence.append(word[0])
                sentence_idx.append(word_idx[0])
