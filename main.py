...
from utils.io import vis_results, load_results, save_test_results, txt_results
...
def main(state):
    ...
    if state.mode == 'train':
        ...
        if state.phase == 'train':
            ...
            txt_results(state, load_results(state, device='cpu')[-1:], state.get_save_directory())
            print("Done Writing Learnt Embeddings from the last step to: ", state.get_save_directory())
