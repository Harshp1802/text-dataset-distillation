...
def evaluate_models(state, models, param_list=None, test_all=False, test_loader_iter=None):
    ...
    with torch.no_grad():
        for i, example in enumerate(test_loader_iter):
            ...
            for k, model in enumerate(models):
                ...
                pred_all_k[k] = torch.cat((pred_all_k.get(k, torch.tensor([])), pred.cpu()), dim=0)
                target_all_k[k] = torch.cat((target_all_k.get(k, torch.tensor([])), target.cpu()), dim=0)
    ...
    # get macro f1 score
    f1_all = []
    langs = {0: 'Ar', 1: 'En', 2: 'Fr', 3: 'De', 4: 'Hi', 5: 'It', 6: 'Pt', 7: 'Es'}
    for k in range(n_models):
        pred_all = pred_all_k[k]
        target_all = target_all_k[k]
        f1_all.append(f1_score(target_all.cpu().numpy(), pred_all.cpu().numpy(), average='macro'))
        # calculate f1 score for every 869 entries in target_all
        lang = 0
        logging.info('Individual Language F1 Scores: ')
        print('Individual Language F1 Scores: ')
        for i in range(0, len(target_all), 870):
            f1_lang = f1_score(target_all[i:i+870].cpu().numpy(), pred_all[i:i+870].cpu().numpy(), average='macro')
            print(langs[lang] + ': ' + str(f1_lang))
            logging.info(langs[lang] + ': ' + str(f1_lang))
            lang+=1
